...
export function removePauseFromEnd(t: TactonInstruction[]) {
	...
	const instructions = t.filter(inst => { return isInstructionSetParameter(inst) == true; }) as InstructionSetParameter[];
...
export function turnOffAllOutputs(t: TactonInstruction[], lastModified: number) {
...
export function processTactonInstructions(tacton: Tacton, lastModified: number) {
	turnOffAllOutputs(tacton.instructions, lastModified);
	removePauseFromEnd(tacton.instructions);
}
